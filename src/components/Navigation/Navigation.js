...
function Navigation(props) {
  const {
    isLoggedIn, location, isNavOpen, setNavOpen, handleLogOut, handleLogIn
  } = props;
  ...
  return (
    <nav className={`navigation ${isNavOpen && 'navigation_active'} ${isOnSavedNewsPage && 'navigation__type_saved'}`}>
      <Link
        to='/'
        className={`navigation__home link ${isOnSavedNewsPage && 'navigation__home_type_saved'}`}
...

      {
        ...
        <Link
          to='/saved-news'
          className={`navigation__saved link ${isOnSavedNewsPage && 'navigation__saved_type_saved'}`}
          onClick={() => { setNavOpen(false) }}
        >Saved articles</Link>
      }
      ...
      >
        {isLoggedIn ?
          <>
            <p className={`navigation__log-title ${isOnSavedNewsPage && 'navigation__log-title_type_saved'}`}>{currentUser.name}</p>
